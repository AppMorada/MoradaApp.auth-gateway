...
build_proj() {
    echo "${INFO_FLAG} Applying new changes on ./dist!"
    pnpm build
    echo "${INFO_FLAG} Generating metadata content..."
    gen_metadata functions "*.spec.ts" "./project-metadata.dev.json"
}

echo "${INFO_FLAG} Starting build in watch mode..."
echo "${INFO_FLAG} Watching files in ./functions"
...
while true; do
    if test -f ./project-metadata.dev.json; then
        ...
        if [ "${SHA256}" != "${OLD_SHA256}" ]; then
            build_proj
            echo "${INFO_FLAG} Listening for file changes..."
        fi

        sleep 1
    else
        echo "${WARN_FLAG} No metadata content was found, building the project..."
        build_proj
        echo "${INFO_FLAG} Listening for file changes..."
    fi
done
